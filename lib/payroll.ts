 ...
 * referencial padrão dos pagamentos — não pelos dias reais do mês. */
 ...
 * nesse período inteiro, então o mês inteiro (30 dias, referencial padrão)
 * conta como não trabalhado (senão fechar um mês anterior à admissão
 * mostrava o salário cheio, como se ele tivesse trabalhado o mês todo).
 */
export function admissionUnworkedDays(hireDate: Date | null, periodEnd: Date): number {
  ...
  return Math.max(0, hireDate.getUTCDate() - 1);
}

/**
 * Fração do salário do mês a pagar quando o funcionário foi admitido no meio
 * do mês: 1 quando trabalhou o mês todo, menos quando entrou depois do dia 1.
 * Os dias trabalhados usam o fim de mês de verdade (28-31, pra não perder o
 * dia 31 em mês longo), mas o salário/dia sempre divide por 30 — é o
 * referencial fixo dos pagamentos, não os dias reais do mês.
 */
export function admissionProrationFactor(hireDate: Date | null, periodEnd: Date): number {
  ...
  const workedDays = daysInMonthUTC(periodEnd) - (hireDate.getUTCDate() - 1);
  return Math.max(0, workedDays) / 30;
}
