...
export function DayOffsSection({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Folgas</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        {weeklyDayOff !== null ? (
          ...
          <p className="text-xs text-muted-foreground">
            Sem folga fixa semanal cadastrada. Defina uma editando o cadastro do funcionário, ou
            registre folgas avulsas abaixo pra não confundir com falta na hora de fechar o
            pagamento.
          </p>
        )}
        <form action={handleSubmit} className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex flex-1 flex-col gap-1">
            ...
            <Input id="dayoff-reason" name="reason" placeholder="Ex: folga semanal" className="h-9" />
          </div>
              ...
              {dayOffs.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="text-center text-neutral-500">
                    Nenhuma folga registrada ainda.
                  </TableCell>
                </TableRow>
              )}
              {dayOffs.map((dayOff) => (
                <TableRow key={dayOff.id}>
                  ...
                  <TableCell className="text-right">
                    <Button
                      ...
                      disabled={isPending}
                      onClick={() => {
                        if (confirm("Excluir esta folga?")) handleDelete(dayOff.id);
                      }}
                    >
                      Excluir
                    </Button>
