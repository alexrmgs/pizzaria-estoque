...
type RecipeIngredientRow = { key: string; ingredientId: string; quantity: string };
...
export function RecipeDialog({
  ...
  const [rows, setRows] = useState<RecipeIngredientRow[]>(() =>
    recipe && recipe.ingredients.length > 0
      ...
      : [{ key: "row-0", ingredientId: "", quantity: "" }],
  );
  ...
  function addRow() {
    setRows((prev) => [
      ...prev,
      { key: `row-${prev.length}-${Date.now()}`, ingredientId: "", quantity: "" },
    ]);
  }
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <form action={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <Input id="name" name="name" defaultValue={recipe?.name} placeholder="Ex: Calabresa" required />
          </div>
