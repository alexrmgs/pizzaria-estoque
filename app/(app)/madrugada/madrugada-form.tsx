...
import { createMadrugadaPayment } from "./actions";
...
export function MadrugadaForm({
  ...
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const result = await createMadrugadaPayment(undefined, formData);
      if (result?.error) {
        toast.error(result.error);
      } else {
        toast.success("Pagamento de madrugada lançado.");
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Lançar pagamento de madrugada</CardTitle>
      </CardHeader>
      <CardContent>
        <form ref={formRef} action={handleSubmit} className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex flex-1 flex-col gap-1">
            ...
            <Input id="description" name="description" placeholder="Ex: abriu a loja 5h" className="h-9" />
          </div>
